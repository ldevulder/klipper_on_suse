...
if grep -Fqs "sonic" /etc/openwrt_release; then
    IS_SONIC_PAD_OS=1
    # On the Sonic Pad, we always want the path to be /usr/share, this is where the rest of the klipper stuff is.
    HOME="/usr/share"
fi

# Next, we try to detect if this OS is an openSUSE.
# openSUSE is a Linux distribution like Debian but uses a different package manager.
IS_SUSE_OS=0
if grep -Eqs "ID_LIKE=.*\<suse\>" /etc/os-release; then
    IS_SUSE_OS=1
fi
...
install_or_update_system_dependencies()
{
    ...
    if [ "$IS_K1_OS" -eq 1 ]; then
        ...
        sudo zypper ref -s 1>/dev/null 2>/dev/null || true
	...
	# NOTE: libjpeg62-devel and libjpeg8 cannot be installed ath same time, kept 6.2 for compatibility
        sudo zypper -n install \
            zlib-ng-devel \
	    zlib-ng-compat-devel \
	    libjpeg-turbo \
	    libjpeg62-devel \
	    python3-virtualenv 2>/dev/null || true
    else
